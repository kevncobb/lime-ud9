    ...
    if ($webform_submission) {
      ...
      if (isset($templateTask['data']['show_edit_form']) && $templateTask['data']['show_edit_form'] == 1) {
        ...
        if($sitewideToken != '') {  //Use a queue Token in the URL.  Webform Form Alter supports the sitewide token as a query parameter
          ...
          if($ui_toolbar) {
            $url = Url::fromUserInput('/admin/webform/manage/' . $webformMachineName . '/submission/' . $sid . '/edit', ['query' => [$sitewideToken => $queueToken]]);
          }
          else {
            $url = Url::fromUserInput('/admin/structure/webform/manage/' . $webformMachineName . '/submission/' . $sid . '/edit', ['query' => [ $sitewideToken => $queueToken]]);
          }
        }
